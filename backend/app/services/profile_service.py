...
class ProfileService:
    def get_profile(self, db: Session, user_id: str):
        ...
        reject_count = sum(1 for a in assessments if a.decision == "reject")
        assessment_ids = [a.id for a in assessments]
        top_rules = []
        if assessment_ids:
            rows = db.query(RiskRule.rule_name, RiskRule.rule_code, RuleHit.rule_id).join(RuleHit, RuleHit.rule_id == RiskRule.id).filter(RuleHit.assessment_id.in_(assessment_ids)).all()
            counter = {}
        ...
        return {
            ...
            "recent_assessments": [{"id": a.id, "risk_score": float(a.risk_score), "risk_level": a.risk_level, "decision": a.decision, "created_at": a.created_at} for a in assessments],
