...
class DashboardService:
    def dashboard(self, db: Session, start_date=None, end_date=None):
        ...
        ranking_rows = db.query(RiskRule.rule_name, RiskRule.rule_code, func.count(RuleHit.id)).join(RuleHit, RuleHit.rule_id == RiskRule.id).filter(RuleHit.created_at >= start, RuleHit.created_at <= end).group_by(RiskRule.rule_name, RiskRule.rule_code).order_by(func.count(RuleHit.id).desc()).limit(10).all()
        return {
            "event_total": event_total,
            "high_risk_rate": round(high_count / len(assessments), 4) if assessments else 0,
